...
extern bool single_foot ;
extern bool angleEKF ;
...
//#endif
	...
	extern  unsigned int TickNumber ;  
...
//fstream aces;
	...
	switch(switchnumb)
	{
		...
		case 3://formal communication
			while(1)
			{	
				tempfile.open("C:/Users/Robotlab/Desktop/ADAMSmodel_hit_mech/test.txt",ios::in);
				...
				if(M2C_ID==127)
				{
					...
					tempfile.open("C:/Users/Robotlab/Desktop/ADAMSmodel_hit_mech/test.txt",ios::out| ios::trunc);
					for(int i=0;i<datasize;i++)
					{
						tempfile<<data[dataindexstart+i]<<"\t";
					}
		...
		case 5://formal communicationz
			...
			while(1)
			{	
				tempfile.open("C:/Users/Robotlab/Desktop/ADAMSmodel_hit_mechlll/test.txt",ios::in);
				for(int g=0;g<datasize+1;g++)
				{
					if(g==0)
						tempfile>>tempZMP[0];
				...
				if(M2C_ID==127)
				{
					...
					tempfile.open("C:/Users/Robotlab/Desktop/ADAMSmodel_hit_mechllll/test.txt",ios::out| ios::trunc);
					for(int i=0;i<datasize;i++)
					{
						tempfile<<data[i]<<"\t";
					}
...
void C2MLoadTrajMOD(int datanumb, double data[],int dataindexstart, fstream &tempfile, double *controlinput )
{
	     //controlinput[0]  waist 
	     //controlinput[1]  arm
	    
	     double ARM_angle = 30*PI/180.0;
		 double ARM_top = 25*PI/180.0;
		 double rotate_count = 300;
		 int start_count = 800;
		 double omega  ;
		 double freq = 2.0/6000.0  ;
		 double counter  ;
		


	     // Biped Traj & Footpad Location (projection of ankle posi. on the ground)
			switch(datanumb){
			case 34:
				//處理腳的12軸
					...
					data[0]=gKineAll.FKLLeg->theta[1];
					data[1]=gKineAll.FKLLeg->theta[2];  
					data[2]=gKineAll.FKLLeg->theta[3];
					data[3]=gKineAll.FKLLeg->theta[4];
					data[4]=gKineAll.FKLLeg->theta[5];
					data[5]=gKineAll.FKLLeg->theta[6];

					data[6]=gKineAll.FKRLeg->theta[1];
					data[7]=gKineAll.FKRLeg->theta[2];
					data[8]=gKineAll.FKRLeg->theta[3];
					data[9]=gKineAll.FKRLeg->theta[4];
					data[10]=gKineAll.FKRLeg->theta[5];
					data[11]=gKineAll.FKRLeg->theta[6];
				
					data[12]=0;//adams waist的 roll方向
				    data[13]=controlinput[0]; //adams waist的 pitch方向(mo.14) 角度值
					...
					if (single_foot!=1)
					{
					if ((TickNumber>2000) && (TickNumber<8000))
					{

				    counter = (TickNumber-2000);
						
					data[14] = (ARM_top * sin ((omega*counter)))  +controlinput[1];//左手
					data[20] = -(ARM_top * sin ((omega*counter))) +controlinput[1];//右手
				    ...
				    //動30度
					else if ((TickNumber<rotate_count+start_count) && (TickNumber>start_count))
					{
                    //data[14] =  -(ARM_angle* (TickNumber-start_count))/rotate_count; 
					//data[20] =  -(ARM_angle* (TickNumber-start_count))/rotate_count; 

					data[17] =  -(ARM_angle* (TickNumber-start_count))/rotate_count; //arm18 左中
					data[23] =  -(ARM_angle* (TickNumber-start_count))/rotate_count; //arm24 右中
					}
					...
					else // single mode  shoulder and elbow both 
					{
					...
					data[14] =  ((ARM_angle* (TickNumber-start_count))/rotate_count)+controlinput[1]; 
					data[20] =  ((ARM_angle* (TickNumber-start_count))/rotate_count)+controlinput[1]; 
					
					data[17] =  -(2*ARM_angle* (TickNumber-start_count))/rotate_count; //arm18 左中
					data[23] =  -(2*ARM_angle* (TickNumber-start_count))/rotate_count; //arm24 右中
					}

					else
					{
						
						data[14] = ARM_angle+controlinput[1];
						data[20] = ARM_angle+controlinput[1];
