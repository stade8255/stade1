...
void Kine::GenSwingTrajMod(double v0, double a0, double x1_percentage, double z1, double v1, double xy2, double z2, double v2, double a2, int Np, int KeepPosCount, double* resultXY, double* resultZ)
{
	...
	******************************************************************/
	...
	if (RunDynamics == 1 && cogestimate == 0)	//WZ
	{
		NWait = 0;
	}
	...
	******************************************************************/
	...
	while (1)
	{
		...
		for (int i = 0 ; i < 9 ; i++)
		{
			...
			else if( RunDynamics == 1 && cogestimate == 0)
				TarRotMBody[i]=	tRFixed[i];
			else
		...
		//	system("pause");
			//for (int i = 0 ; i < 3 ; ++i){
			//	SwingErr[i] *= MaxSwingError/NormXYZD(SwingErr);	// WZ0419限制最大速度
			//}
		}
		...
		//	system("pause");
			//for (int i = 0 ; i < 3 ; ++i){
			//	COGErr[i] *= MaxCOGError/NormXYZD(COGErr);	// WZ0419限制最大速度
			//}
		}
		...
		//	system("pause");
			//for (int i = 0 ; i < 3 ; ++i){
			//	temp_vec2[i] *= MaxRotationError/NormXYZD(temp_vec2);	// WZ0419限制最大速度
			//}
		}
		...
		//	system("pause");
			//for (int i = 0 ; i < 3 ; ++i){
			//	temp_vec1[i] *= MaxRotationError/NormXYZD(temp_vec1);	// WZ0419限制最大速度
			//}
		}
		if(IKMode == 0)	// 解全身
		{
...
void Kine::IMUSensorData(int LogCount,float *accex ,float *accey,float *accez, float *angvelx,float *angvely,float *angvelz, float *roll, float *pitch, float *yaw, float *rollF, float *pitchF, float *yawF)
{
	//從ADAMS讀入sensor data
...
//void sensorfilter( float *datain , float  *dataout ) {
	...
	////以下測試用
	//	fstream Fx;
	//	Fx.open("TestAdamsIMU.txt",ios::app);	
	//	for(int i = 0 ; i < 12 ; i++)	
	//		Fx << AdamsIMU[i] << "\t";
	//	Fx << endl;
	//	Fx.close();
...
void Kine::IMUSensorData_hsuan(int FlagSim,int LogCount,float *IMUaccex ,float *IMUaccey,float *IMUaccez, float *IMUangvelx,float *IMUangvely,float *IMUangvelz ,
	double *LFX,float *IMUpitch ,double *waistthetadot){
	...
	//{
	//	*(LFX+LogCount) = -AdamsFS[6];

	//}
	//else if(gKineAll.selSupport[gKineAll.stepIndex]==0)
	//{
	//	*(LFX+LogCount) = -AdamsFS[0];
	//}	
	//else
	//{
	//	*(LFX+LogCount) = -(AdamsFS[0]+AdamsFS[6]);
	//}
	//
	//
...
void Kine::ForceSensorData(int FlagSim, int FlagGo, int FSCount, double *FS_DataL, double *FS_DataR)
{
	...
	******************************************************************/
	...
	if(FlagGo==0){	// Go按下 CrdAll開始有值	
		...
		//Weight_cali = ForceDataKFL[FSCount*6+2] + ForceDataKFR[FSCount*6+2];
	}
...
void Kine::Distributor(double *ZMPxd, double *ZMPyd, int FKCount, int Tick, double Torq_gain)
{
	...
	******************************************************************/
	...
	// 力矩積分補償
	//Torq_err[0] = Torq_gain*(ZMP[0] - FS_ZMP[LogCount*2])*Weight_cali;	// x
	//Torq_err[1] = Torq_gain*(ZMP[1] - FS_ZMP[LogCount*2+1])*Weight_cali;	// y
	//Torq_I[0] += Torq_err[0];
	//Torq_I[1] += Torq_err[1];
	...
	//Ratio_total = RatioRe[LogCount*2] + RatioRe[LogCount*2+1];
	//Torq_del[0] = RatioRe[LogCount*2+1] * Torq_err[0] / Ratio_total;	// L Pitch
	//Torq_del[1] = RatioRe[LogCount*2] * Torq_err[1] / Ratio_total;	// L Roll
	//Torq_del[2] = RatioRe[LogCount*2+1] * Torq_err[0] / Ratio_total;	// R Pitch
	//Torq_del[3] = RatioRe[LogCount*2] * Torq_err[1] / Ratio_total;	// R Roll	
	...
	Torq_del[1] = RatioRe[Tick*2] * Torq_gain*Torq_I[1] / Ratio_total;	// L Roll
	Torq_del[2] = RatioRe[Tick*2+1] * Torq_gain*Torq_I[0] / Ratio_total;	// R Pitch
