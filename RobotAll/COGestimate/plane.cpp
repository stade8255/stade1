...
cPlaneEKF::cPlaneEKF() 
{  	 
	ZMPflag = 0;	// 是否把ZMP加入ekf算COG

	if ( ZMPflag == 0 )
		setDim(3, 1, 3, 2, 2);  //設定matrix dimention  n state = 4 (X = A X   X vector 的大小)   m (measurement vector  的大小)
	else
...
void cPlaneEKF::makeBaseR()  //measurement noise  (依據每個sensor會有不同) 測量方式:將sensor平放，獲得covariance
{
	...
	else
	{
		...
		if ( ZMPflag == 0 )
		{
			R(1,1) = 0.1;   // 由joint encoder 回算的COG 位置
			R(2,2) = 0.000022;  //IMU 量測的 COG 加速度 
			// 數值用哲瑄的，可重測
		}
		else
		{
