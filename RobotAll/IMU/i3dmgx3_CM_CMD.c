...
int ReadNextRecord(int portNum, I3dmgx3Set* pRecord, BYTE* cmd_return){
	...
	unsigned char Bresponse[79] = {0}; //Current max buff request is 42
	unsigned short wChecksum = 0;
	unsigned short wCalculatedCheckSum = 0;
	HANDLE portHandle;
	BOOL found = FALSE;
	...
	while( !found) {
		...
		if (cmd_chk == 0xC4) //Continuous mode confirmation packet
			found = TRUE;
		if (cmd_chk == 0xCC) //Acceleration & Angular & Mag & Rotation Matrix packet
			found = TRUE;
		if (cmd_chk == 0xCF) //Euler Angle
			found = TRUE;
		if (cmd_chk == 0xC6) //Euler Angle
			found = TRUE;
		if (cmd_chk == 0xDF) //Euler Angle
			found = TRUE;
		if (iTry++ >= iMaxTry);
		    found = TRUE;
   		...
   		switch(cmd_chk)	{
			case 0xc2:
		        ...
		        case 0xc4:
				  ...
				  break;
		     case 0xcc:
				  status = receiveData(portNum, Bresponse, 78);
			...
			case 0xcf:
				  ...
				  pRecord->timer = ((((((Bresponse[24]) << 8) & Bresponse[25]) << 8) & Bresponse[26]) << 8) & Bresponse[27];
				  wChecksum = convert2ushort(&Bresponse[28]);
	              wCalculatedCheckSum = i3dmgx3_Checksum(&Bresponse[0], 28) + 0xCF; 		
				  break;
            case 0xc6:
				  status = receiveData(portNum, &Bresponse[0], 42);
				  pRecord->timer = ((((((Bresponse[36]) << 8) & Bresponse[37]) << 8) & Bresponse[38]) << 8) & Bresponse[39];
				  wChecksum = convert2ushort(&Bresponse[40]);
	              wCalculatedCheckSum = i3dmgx3_Checksum(&Bresponse[0], 40) + 0xC6; 		
				  break;
			case 0xdf:
				  status = receiveData(portNum, &Bresponse[0], 22);
				  pRecord->timer = ((((((Bresponse[16]) << 8) & Bresponse[17]) << 8) & Bresponse[18]) << 8) & Bresponse[19];
				  wChecksum = convert2ushort(&Bresponse[20]);
	              wCalculatedCheckSum = i3dmgx3_Checksum(&Bresponse[0], 20) + 0xdf; 		
				  break;
	...
	if (cmd_chk != 0xc4) {   
	     for (i=0; i<3; i++) {
				...
				if (cmd_chk == 0xDF)
					pRecord->setB[0] = FloatFromBytes(&Bresponse[12]);
				if (cmd_chk == 0xC2 ||cmd_chk == 0xCB || cmd_chk == 0xD3 || cmd_chk == 0xCF || cmd_chk == 0xC6 || cmd_chk == 0xCC) //
					pRecord->setB[i] = FloatFromBytes(&Bresponse[12 + i*4]); // Ang rates x y z
				if (cmd_chk == 0xCB || cmd_chk == 0xD3 || cmd_chk == 0xC6) //
					pRecord->setC[i] = FloatFromBytes(&Bresponse[24 + i*4]); // Mag rates x y z   
				if (cmd_chk == 0xCC) 
				{
					pRecord->setC[i] = FloatFromBytes(&Bresponse[24 + i*4]);
					pRecord->setD[i] = FloatFromBytes(&Bresponse[36 + i*4]);
					pRecord->setE[i] = FloatFromBytes(&Bresponse[48 + i*4]);
					pRecord->setF[i] = FloatFromBytes(&Bresponse[60 + i*4]);
				}
		}
	}
 ...
 *                          an error code.
 *--------------------------------------------------------------------*/
int i3dmgx3_getDeviceIdentiy(int portNum, char flag, unsigned char *snid) {
